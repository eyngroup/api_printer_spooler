      ...
      if (len(trama) > 69):
        try:
          ...
          if (len(_arrayParameter) > 1):
            self._setSubTotalBases(Util().do_value_double(_arrayParameter[0][3:]))
            self._setSubTotalTax(Util().do_value_double(_arrayParameter[1][1:]))
            self._setDataDummy(_arrayParameter[2][1:])
            self._setQuantityArticles(int(_arrayParameter[3]))
            self._setAmountPayable(Util().do_value_double(_arrayParameter[4][1:]))
