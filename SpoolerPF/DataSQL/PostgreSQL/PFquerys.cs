    ...
    public class OdooQuerys
    {
        ...
        public string AccountMove(string cPFSerial, int nLimit)
        {
            ...
            string queryAccountMove = string.Format
                    ...
                    "DIARIO.TYPE = 'sale' AND DOC.REF IS NOT NULL AND DIARIO.SERIAL_PRINTER = '{0}' ORDER BY DOC.ID LIMIT '{1}';", cPFSerial, nLimit);
        ...
        public string AccountMove(int nID)
        {
            /* Estrutura de Datos:
            ...
            string queryAccountMove = string.Format
                   ...
                   "INNER JOIN PUBLIC.RES_PARTNER AS CLI ON CLI.ID = DOC.PARTNER_ID " +
                   "INNER JOIN PUBLIC.RES_USERS AS VEN ON VEN.ID = DOC.INVOICE_USER_ID " +
                   "WHERE DOC.NUM_DOC_FISCAL IS NULL AND DOC.ID = '{0}' ;", nID);
        ...
        /// <returns>string query</returns>
        public string AccountMoveLine(int nID)
        {
            /* Estrutura de Datos:
            ...
            string queryAccountMoveLine = string.Format
                    ...
                    "WHERE LIN.EXCLUDE_FROM_INVOICE_TAB = FALSE AND LIN.MOVE_ID = '{0}' ORDER BY LIN.ID;", nID);
        ...
        /// <param name="cReferencia">Referencia de la Operacion</param>
        /// <returns>string query</returns>
        public string PosOrder(int nID)
        {
            /* Estrutura de Datos:
            ...
            */
            string queryPosOrder = string.Format
                    ...
                    "AND POS.ACCOUNT_MOVE = '{0}' ORDER BY IS_DOLLAR_PAYMENT ;", nID);
            return queryPosOrder;
        }
        #endregion

        #region UPDATE
        public string UpdateAccountMove(string cInvoice, int nID)
        {

            if (string.IsNullOrEmpty(cInvoice) || nID == 0)
            {
                return string.Empty;
            }
