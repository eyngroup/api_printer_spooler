...
async function PrinterInvoiceAPI(parent, { params }) {
    try {
        if (!params.url.startsWith('http')) {
            throw new Error('Configuración de URL de impresora no válida');
        }
        ...
        xhr.onload = function () {
            ...
            try {
                ...
                alert(`Error de conexión con la impresora: ${xhr.statusText}`);
                return;
            }
...
- Error: ${response.data.Error}`;
        ...
        xhr.onerror = function () {
            ...
            alert("Error al conectar con la impresora fiscal.\nVerifique:\n- La configuración en el diario\n- Que el servicio esté activo\n- La conexión de red");
        };
    } catch (error) {
