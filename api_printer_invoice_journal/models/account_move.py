...
class AccountMove(models.Model):
    ...
    def _prepare_payments_data(self):
        ...
        # Usar la configuración del diario de ventas de la factura
        sale_journal = self.journal_id
        default_code = sale_journal.aps_default_payment_code or "01"
        default_name = sale_journal.aps_default_payment_name or "EFECTIVO"
        payments = []
    ...
    def button_send_to_printer(self):
        ...
        # Usar la configuración del diario de ventas específico
        sale_journal = self.journal_id
        if not sale_journal.aps_ip or not sale_journal.aps_port:
            raise UserWarning("Printer configuration not found in the sales journal")

        api_url = f"{sale_journal.aps_ip}:{sale_journal.aps_port}/api/printers"
